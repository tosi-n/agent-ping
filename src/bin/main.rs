use agent_ping::channels::{slack as slack_channel, telegram as telegram_channel, whatsapp as whatsapp_channel};
use agent_ping::config::{load_config, resolve_database_url, Binding, Config};
use agent_ping::db::{self, DbKind};
use agent_ping::outbox;
use agent_ping::session;
...
struct AppState {
    ...
    pool: AnyPool,
    http: reqwest::Client,
    ws_tx: broadcast::Sender<ws::WsEvent>,
    db_kind: DbKind,
}
...
fn resolve_binding(
    ...
    let mut best: Option<(i32, &Binding)> = None;
